...
$(document).ready(function(){
    ...
    $("#t_Email").formValidator({onshow:"6-100个字符",onfocus:"6-100个字符",oncorrect:"恭喜你,你输对了",defaultvalue:"@"}).inputValidator({min:6,max:100,onerror:"长度非法"}).regexValidator({regexp:regexEnum.email,onerror:"格式不正确"});
    $("#t_iphone").formValidator({onshow:"请输入手机号",onfocus:"11位",oncorrect:"通过"}).regexValidator({regexp:regexEnum.mobile,onerror:"格式不正确"});
});
